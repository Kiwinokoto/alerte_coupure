...
- **Reference:** `dev/android-v1` code at `e3c6a74` before this handoff update; workstation clone matched `origin/dev/android-v1` before editing and was otherwise clean.
- **Completed:** retry idempotence, persistent Android outbox/replay, persistence-before-webhook-validation and Gradle wrapper remain published. This pass hardened outbox corruption handling: each successful write keeps the previous valid JSON snapshot, and a malformed primary queue is recovered from that snapshot instead of silently appearing empty. A recovered stale event can be replayed, which is safe because server `event_id` idempotence is already in place.
- **Tests:** `git diff --check` passed. Two attempts to rerun the Android Gradle build in disposable JDK 17 containers stalled during Gradle daemon startup/configuration and were terminated; no compile failure was reported, but this pass therefore does **not** claim a fresh Android build. The preceding pass had `testDebugUnitTest assembleDebug` BUILD SUCCESSFUL, with no unit-test sources.
- **Device:** do not replace the currently armed OnePlus package unattended. The next meaningful validation remains an attended outbox/replay scenario on the physical OnePlus.
- **VPS:** `/opt/powerwatch` was clean at `b9df88e`, intentionally behind GitHub; `powerwatch-api` was healthy. No restart/redeploy and no live DB migration were performed.
- **Next:** (1) diagnose/retry the containerized Gradle build without changing the workstation system; (2) attended OnePlus test: install current debug APK, re-arm if needed, make backend unavailable or endpoint blank, create a real power event, restart process/app, restore backend/config and verify exactly one server record; (3) add lightweight automated outbox persistence/recovery/replay tests; (4) add a bounded retention policy that does not discard critical power events casually; (5) continue degraded-backend UI/state work; (6) leave actual SMS delivery for an explicit later decision/test.
- **Risk/debt:** SharedPreferences JSON remains a deliberately small V1 store. The one-generation backup avoids silent loss from a malformed primary queue but is not a transactional database; if both primary and backup are malformed, `pending()` still returns empty. There is still no retention cap. Gradle 9 deprecations remain non-blocking debt.
