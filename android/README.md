...
- **Reference:** `dev/android-v1` code at `f460b80` (`Harden critical delivery fallback preparation`). GitHub and workstation were aligned before commit. VPS `/opt/powerwatch` remains intentionally at `b9df88e`; `powerwatch-api` is healthy and no server deploy/restart was performed.
...
- **Tests:** containerized JDK 17 + cached Android SDK 36 + persistent `~/.android/debug.keystore`: `./gradlew --no-daemon clean testDebugUnitTest assembleDebug` → **BUILD SUCCESSFUL** (44 tasks). **12/12 Android unit tests pass**, covering outbox persistence/recovery/retention, backend-state preferences, fallback eligibility/idempotence/clearing, and critical retry policy. `git diff --check` passed before commit.
- **Device now:** OnePlus is back to `armed=true`, mains present, original endpoint/token restored, backend health `OK`, foreground `MonitorService` running, outbox empty and no fallback event pending.
- **Next:** (1) controlled VPS/backend rollout of the already-tested server idempotence migration only when deployment is explicitly authorized, then verify duplicate replay against the migrated live DB; (2) decide and implement the actual direct fallback transport (SMS is the leading V1 option) while keeping paid sending disabled until an explicit test; (3) finish remaining Android/OxygenOS reliability checks such as task swipe/removal, reboot/update/recovery edge cases and notification presence; (4) release/signing/configuration hardening for restaurant handoff.
- **Risk/debt:** the live container still runs the pre-idempotence schema: live SQLite has no `events.event_id` column or unique event-id index. `FallbackTracker` currently marks eligibility only after the initial critical delivery retry sequence has exhausted; an actual SMS sender is not implemented yet. SharedPreferences remains a deliberately small V1 persistence layer. Gradle 9 deprecation warnings remain non-blocking.
