...
- **Reference:** `dev/android-v1` code at `ef5f8ad` (`Confirm before disarming PowerWatch`). Workstation clone was clean/synced before this pass. VPS `/opt/powerwatch` remains intentionally at `b9df88e`; `powerwatch-api` is healthy.
- **Completed:** performed the first physical-OnePlus outbox/replay validation with the current Android code. With the webhook deliberately blank, `monitoring_started` and a simulated confirmed `power_lost` were persisted; after an Android `am force-stop` and explicit restart the same outbox survived; simulated power restoration added `power_restored`; restoring the real endpoint drained all 5 pending events successfully. The live server recorded one `power_lost` and one `power_restored`, and all 10 new payloads observed during the test window had distinct `event_id` values.
- **Update robustness:** discovered that APK replacement stops the foreground service. Added `ACTION_MY_PACKAGE_REPLACED` handling to the existing startup receiver. A same-signature `adb install -r` on the armed OnePlus changed the app PID and PowerWatch automatically returned with `MonitorService` running and a new `MONITORING_STARTED` entry.
- **Disarm UX:** the armed-state action is now explicitly **Désarmer et quitter** and opens a confirmation explaining that monitoring will stop; cancelling/closing the activity still leaves the foreground monitor running. The updated APK builds successfully and is installed on the OnePlus; the device remains armed after the update.
- **Build/signing:** containerized JDK 17 + cached Android SDK 36: `./gradlew --no-daemon testDebugUnitTest assembleDebug` → **BUILD SUCCESSFUL**; all 4 outbox tests pass. The previously installed debug APK used an ephemeral debug signer, so the first update was signature-incompatible. The phone data was backed up before the one-time reinstall, core preferences/event log were restored, and a stable workstation `~/.android/debug.keystore` is now used outside the repo. Future container builds must mount that directory; never commit the keystore.
- **Device now:** OnePlus 7T Android 11 is armed again, reports mains present, uses the original endpoint/token, the persistent outbox is empty, and the foreground service is running after the package-replacement test.
- **Next:** (1) controlled VPS/backend rollout of the already-tested server idempotence migration when deployment is explicitly authorized, then verify duplicate replay against the migrated live DB; (2) implement the degraded backend/network state in the Android UI; (3) design/test direct-SMS fallback without sending a paid SMS until explicitly requested; (4) continue the remaining Android/OxygenOS reliability matrix.
- **Risk/debt:** the current live container still runs the pre-idempotence schema: live SQLite has no `events.event_id` column or unique event-id index. The physical replay test therefore validates Android persistence/replay and observed no client-side duplicate delivery, but it does **not** yet prove server-side deduplication in production. SharedPreferences JSON remains a deliberately small V1 store; if both primary and one-generation backup are malformed, `pending()` returns empty. Critical power transitions remain intentionally uncapped. Robolectric SDK 35 tests complement but do not replace Android 11 device testing.
- **Migration/deploy note:** server idempotence adds `events.event_id` plus a unique partial index through startup `init_db()`. This change is additive/backward-compatible in the tested code but remains **not applied to the live DB**; no server restart/deploy was performed in this pass.
