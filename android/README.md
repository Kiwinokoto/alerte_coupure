...
- **Reference:** `dev/android-v1` code through `93449b6` before this handoff update. The personal VPS deployment clone was clean and was fast-forwarded from `b9df88e` to `93449b6` during the controlled backend rollout.
...
- **Next:** (1) decide and implement the actual direct fallback transport (SMS is the leading V1 option) while keeping real/paid sending disabled until an explicit test; (2) finish remaining Android/OxygenOS reliability checks, especially reboot/recovery and Wi-Fi/mobile-data transitions; (3) release/signing/configuration hardening for restaurant handoff; (4) configure and test the real outbound alert delivery path, which is still unconfigured on the live backend.
- **Risk/debt:** server idempotence is now live, but the direct fallback transport itself is still absent: critical events are persisted/marked robustly, including after crash/replay, but no SMS is sent yet. Live `/healthz` currently reports `alert_delivery_configured=false`, so the normal outbound alert channel still needs production configuration/testing. Production release signing is also not configured; creating and safeguarding the long-lived release key is a human/security decision. SharedPreferences remains a deliberately small V1 persistence layer. Gradle 9 deprecation warnings remain non-blocking.
