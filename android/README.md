...
- **Reference:** `dev/android-v1` code at `3870fc4` before this handoff update; workstation was clean/synced before the pass. VPS `/opt/powerwatch` was clean at `b9df88e` (behind GitHub) and `powerwatch-api` healthy.
- **Completed:** added the first automated Android tests for `EventOutbox`: persistence/deduplication by `event_id`, selective removal, and recovery of the last valid snapshot when the primary JSON is malformed. Enabled AndroidX for test dependencies and Robolectric 4.16.1; tests deliberately run against SDK 35 because Robolectric's SDK 36 sandbox requires Java 21 while the application build remains on the project's JDK 17 requirement.
- **Tests:** containerized JDK 17 + cached Android SDK 36: `./gradlew --no-daemon testDebugUnitTest assembleDebug` → **BUILD SUCCESSFUL**; all 3 new outbox tests passed and the debug APK assembled. `git diff --check` passed before commit.
...
- **Next:** (1) attended OnePlus outbox/replay scenario above; (2) add a bounded retention policy that preserves critical `power_lost`/`power_restored` events rather than blindly truncating; (3) add a small replay-focused automated seam/test if it can be done without refactoring the networking layer broadly; (4) continue degraded-backend UI/state work; (5) leave actual SMS delivery for an explicit later decision/test.
- **Risk/debt:** SharedPreferences JSON remains a deliberately small V1 store. One-generation backup is now covered by an automated recovery test, but if both primary and backup are malformed `pending()` still returns empty. There is still no retention cap. Robolectric SDK 35 tests exercise the persistence logic but do not replace physical Android 11 validation on the OnePlus. Gradle 9 deprecations remain non-blocking debt.
