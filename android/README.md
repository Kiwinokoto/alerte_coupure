...
- **Tests:** Android CI runs automatically on `dev/android-v1` for Android changes using JDK 17, API 36 and the checked-in Gradle wrapper. The current Android suite contains **18 unit tests**; the latest local `testDebugUnitTest + assembleDebug` pass succeeded, and CI also verifies `assembleRelease` plus a debug APK artifact. Coverage includes outbox persistence/recovery/retention, backend-state preferences, SMS recipient normalization/configuration, fallback eligibility/idempotence/clearing, critical retry policy and critical-first replay ordering. Server suite is **9/9 green**. Production signing remains intentionally unconfigured. `git diff --check` passed before commits.
