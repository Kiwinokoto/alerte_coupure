...
- **Reference:** `dev/android-v1` code at `b1b55c3` (`Track degraded backend and network state`). Workstation clone was clean/synced before this pass. VPS `/opt/powerwatch` remains intentionally at `b9df88e`; `powerwatch-api` is healthy and no server deploy/restart was performed.
- **Completed:** the physical OnePlus outbox/replay scenario is validated: with the webhook deliberately blank, pending events survived process stop/restart, a simulated confirmed `power_lost` and `power_restored` were replayed after restoring the endpoint, the outbox drained, and the live server observed one loss and one restoration. APK replacement while armed is also covered by `ACTION_MY_PACKAGE_REPLACED`, and **Désarmer et quitter** now requires explicit confirmation.
- **Degraded-state UI foundation:** Android now tracks backend health independently from connectivity. `BackendHealth` distinguishes `UNKNOWN`, `OK`, HTTP-level `ERROR`, and `UNREACHABLE`; the last real HTTP contact timestamp is persisted. The UI exposes separate **Réseau** and **Backend/VPS** lines, while keeping the configured alert channel and last event delivery distinct. Changing the webhook resets stale backend health/contact data.
- **Device validation:** current debug APK installed successfully with the persistent workstation debug signer; package replacement automatically resumed the armed foreground service. On the OnePlus 7T Android 11, `ACCESS_NETWORK_STATE` is granted, backend health became `OK`, a backend contact timestamp was recorded, `armed=true`, mains is present, and `MonitorService` remains foreground.
- **Tests:** containerized JDK 17 + cached Android SDK 36 + persistent `~/.android/debug.keystore`: `./gradlew --no-daemon clean testDebugUnitTest assembleDebug` → **BUILD SUCCESSFUL**. **8/8 Android unit tests pass** (4 outbox + 4 backend-state preference tests), and `git diff --check` passed before the code commit.
- **Next:** (1) controlled VPS/backend rollout of the already-tested server idempotence migration only when deployment is explicitly authorized, then verify duplicate replay against the migrated live DB; (2) add the planned green/amber/red/grey visual treatment around the now-structured states without changing their semantics; (3) design/test direct-SMS fallback without sending a paid SMS until explicitly requested; (4) continue the remaining Android/OxygenOS reliability matrix.
- **Risk/debt:** the live container still runs the pre-idempotence schema: live SQLite has no `events.event_id` column or unique event-id index. The physical replay test validates Android persistence/replay but does not yet prove production server deduplication. `UNREACHABLE` means the last backend attempt failed and does not by itself distinguish VPS failure from DNS/routing loss; the separate Android network line provides that local context. `NET_CAPABILITY_VALIDATED` is Android's view of usable Internet, not an end-to-end guarantee. SharedPreferences JSON remains a deliberately small V1 outbox store.
- **Migration/deploy note:** server idempotence adds `events.event_id` plus a unique partial index through startup `init_db()`. This additive/backward-compatible change remains **not applied to the live DB** because no server deploy/restart has been authorized in this pass.
