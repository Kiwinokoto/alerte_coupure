...
- **Next:** (1) provision the user-selected SMS recipient locally on the OnePlus, grant `SEND_SMS`, explicitly enable fallback and perform the first controlled real-SMS test; no recipient number is stored in Git; (2) finish remaining Android/OxygenOS reliability checks, especially reboot/recovery and Wi-Fi/mobile-data transitions; (3) release/signing/configuration hardening for restaurant handoff; (4) configure and test the real outbound alert delivery path, which is still unconfigured on the live backend.
