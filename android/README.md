...
- keep validating the now-working direct-SMS fallback during broader outage/recovery testing; the recipient remains local and is intentionally not committed to Git;
...
- add a transport-neutral network-resilience state machine before considering privileged radio control: treat **mains state** and **network state** independently and define the normal IP path as **any validated Internet transport currently available**, whether Wi-Fi or cellular data. Do not assume Wi-Fi is primary: a site may intentionally run for days or weeks with its router off while the dedicated phone uses a low-cost mobile-data plan. Distinguish transient transport loss / router reboot, Wi-Fi connected without Internet, validated cellular-data Internet, multiple simultaneous transports, and total IP loss. Do not flap routes on the first failure: use repeated probes plus a grace/debounce window for network-only incidents, while a confirmed mains-loss event may use a shorter escalation path. Prefer Android's currently validated default network; when the default path is unhealthy and another already-enabled validated transport exists, explicitly request/use it without changing radio settings. If all IP paths fail, use the independent SMS path. Later productisation may add a configurable metered-data/cost policy, but V1 reliability must not silently prefer Wi-Fi over cellular. Keep automatic Wi-Fi/mobile-data toggling out of the normal app; evaluate Device Owner/root/carrier-level control only if physical reliability tests prove normal Android transport selection insufficient;
...
- **Reference:** `dev/android-v1` code through `4eeb1d4` before this handoff update. The personal VPS backend remains on the idempotence-capable runtime from the controlled rollout; the later Android/UI changes do not require a backend redeploy.
- **Platform roadmap:** keep Android as the only implementation target until the restaurant V1 is reliable. After that, Windows is the first desktop port, Linux should follow closely, and macOS is viable with its own packaging/sleep validation. iOS is not assumed to be an autonomous probe because normal background execution is constrained and public SMS composition requires user interaction. All future probes should share event/outbox/heartbeat/backend semantics while advertising platform capabilities such as direct SMS independently.
...
- **Physical fallback validation:** current code was built and installed on the OnePlus 7T Android 11. Earlier degraded-delivery tests validated critical retries, wake-lock coverage, crash/replay ordering and fallback eligibility. On 2026-09-20 the direct SMS path was then tested end-to-end with the real locally configured recipient: mobile data was disabled, a confirmed power loss was simulated, and the user confirmed receipt of the PowerWatch SMS. The app recorded exactly **1** submitted SMS event, with outbox count **0** and fallback-eligible count **0** after connectivity returned; a later heartbeat again delivered `HTTP 200`. No recipient number is stored in Git or docs. Test-harness lesson: because the Linux workstation itself was using the phone hotspot, disabling the phone's mobile data also severed RDC; future offline tests should preserve an independent control path or use USB-only control.
...
- **Degraded-state/UI status:** Android independently displays monitoring, mains, battery optimization, network, backend/VPS health, last backend contact and fallback state using the pale green / amber / red / grey treatment. `BackendHealth` remains `UNKNOWN`, `OK`, HTTP-level `ERROR`, or `UNREACHABLE`. Network status is now transport-neutral: a validated Wi-Fi, cellular-data, Ethernet or VPN path is healthy, and the UI names the active transport instead of implying Wi-Fi is primary. The reusable `NetworkSnapshot` extraction is the first small foundation for the later network-resilience state machine. SMS configuration UX was also hardened: IME Done/Enter saves, dismisses the keyboard and clears focus, while the one-second UI refresh preserves scroll position during configuration editing.
- **Tests:** Android CI runs automatically on `dev/android-v1` for Android changes using JDK 17, API 36 and the checked-in Gradle wrapper. The current Android suite contains **21 unit tests** after adding transport-label coverage. A clean-copy, non-root Docker/JDK 17 validation ran `testDebugUnitTest + lintDebug + assembleRelease` successfully: **0 lint errors / 27 non-blocking warnings**. This avoids mutating the clone's older root-owned Gradle/Kotlin build artifacts; those stale local artifacts still exist and should not be `sudo`-fixed automatically. Coverage includes outbox persistence/recovery/retention, backend-state preferences, SMS recipient normalization/configuration, fallback eligibility/idempotence/clearing, critical retry policy, critical-first replay ordering and transport-neutral network labels. Server suite is **9/9 green** from the previous backend pass. Production signing remains intentionally unconfigured. `git diff --check` passed before commits.
- **2026-09-21 release-hardening pass:** documented unsigned-vs-distributable release behavior and external signing-key custody; `android/.gitignore` now excludes `*.jks` and `*.keystore`, verified with `git check-ignore`. No key was created or exposed. A direct host Gradle invocation is still unsuitable because the workstation defaults to JDK 11 and old build/cache artifacts include root-owned files from earlier Docker runs; no sudo/chown/cache deletion was attempted. The safe workaround is now proven: rsync the Android tree to a disposable clean directory, run Docker as the normal host UID/GID with JDK 17, a read-only Android SDK and an isolated user-owned Gradle cache. That path completed the full 21-test/lint/release validation successfully.
- **Device now:** physical state was re-checked after the real SMS test. PowerWatch is **disarmed**, the normal non-exported `MonitorService` APK is reinstalled, the simulated battery state has been reset to real hardware (`USB powered=true` while connected), LTE data is connected, webhook configuration is preserved, and the SMS recipient + fallback-enabled setting remain stored locally. ADB works over USB and TCP on the phone hotspot; the persistent Linux udev rule for OnePlus vendor `2a70` removes the previous USB `no permissions` issue.
- **Next:** (1) continue remaining Android/OxygenOS reliability checks, especially reboot/recovery, long screen-off runs and network transitions; (2) verify the SMS input/scroll UX once more during ordinary manual use after the fix; (3) build the next small step of the transport-neutral network-resilience state machine on top of `NetworkSnapshot` without auto-toggling radios; (4) finish release/signing/configuration hardening for restaurant handoff; (5) configure and test the normal outbound alert delivery path, which is still unconfigured on the live backend; (6) optionally build the reusable workstation-side ADB helper described above for future Android projects. Desktop ports remain post-V1.
- **Risk/debt:** direct SMS transport is now physically validated on the OnePlus, but Android `SEND_SMS` remains a sensitive/restricted permission and would require policy review for future Play Store distribution; the current V1 is a dedicated/sideloaded-device workflow. Local event-ID tracking prevents duplicate submission by PowerWatch, but Android accepting a send request is still not the same as carrier delivery confirmation. Live `/healthz` currently reports `alert_delivery_configured=false`, so the normal outbound alert channel still needs production configuration/testing. Production release signing is also not configured; creating and safeguarding the long-lived release key is a human/security decision. SharedPreferences remains a deliberately small V1 persistence layer. Gradle 9 deprecation warnings remain non-blocking.
