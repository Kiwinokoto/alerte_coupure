...
- **Reference:** `dev/android-v1` code at `a1814d7` (`Require confirmation before stopping monitoring`). GitHub and workstation were aligned before commits. VPS `/opt/powerwatch` remains intentionally at `b9df88e`; `powerwatch-api` is healthy and no server deploy/restart was performed.
