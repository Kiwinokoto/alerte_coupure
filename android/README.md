...
- **Reference:** `dev/android-v1` code at `d827b7a`; workstation was clean/synced before this pass. VPS `/opt/powerwatch` was clean at `b9df88e` (behind GitHub) and `powerwatch-api` healthy.
- **Completed:** bounded the persistent outbox's non-critical backlog to the newest 100 events while explicitly preserving all `power_lost` and `power_restored` transitions. This avoids unbounded SharedPreferences growth during prolonged backend/configuration failure without discarding the events that matter most for safety/replay. Added an automated retention test covering both eviction order and preservation of critical transitions.
- **Tests:** containerized JDK 17 + cached Android SDK 36: `./gradlew --no-daemon testDebugUnitTest assembleDebug` → **BUILD SUCCESSFUL** (43 tasks); all 4 outbox tests passed and the debug APK assembled. `git diff --check` passed before commit. The first build attempt used the wrong cached SDK mount and failed only on missing license acceptance; rerunning against the established PowerWatch SDK cache succeeded.
- **Device:** do not replace the currently armed OnePlus package unattended. The next meaningful end-to-end validation remains attended: install current debug APK, re-arm if needed, make backend unavailable or endpoint blank, create a real power event, restart process/app, restore backend/config and verify exactly one server record.
- **Next:** (1) attended OnePlus outbox/replay scenario above; (2) add a small replay-focused automated seam/test if it can be done without refactoring the networking layer broadly; (3) continue degraded-backend UI/state work; (4) design/test direct-SMS fallback, leaving actual SMS delivery for an explicit later decision/test.
- **Risk/debt:** SharedPreferences JSON remains a deliberately small V1 store. One-generation backup is covered by an automated recovery test, but if both primary and backup are malformed `pending()` still returns empty. Critical power transitions are intentionally not capped, so a pathological long-lived failure with repeated real transitions can still grow the store; that is preferable to silently discarding them in V1. Robolectric SDK 35 tests do not replace physical Android 11 validation on the OnePlus. Gradle 9 deprecations remain non-blocking debt.
