...
- **Reference:** `dev/android-v1` code through `891a37f` before this handoff update. The personal VPS backend remains on the idempotence-capable runtime from the controlled rollout; the later Android/UI changes do not require a backend redeploy.
...
- **Degraded-state/UI status:** Android independently displays monitoring, mains, battery optimization, network, backend/VPS health, last backend contact and fallback state using the pale green / amber / red / grey treatment. `BackendHealth` remains `UNKNOWN`, `OK`, HTTP-level `ERROR`, or `UNREACHABLE`. SMS configuration UX was also hardened: IME Done/Enter now saves, dismisses the keyboard and clears focus, while the one-second UI refresh preserves scroll position during configuration editing.
- **Tests:** Android CI runs automatically on `dev/android-v1` for Android changes using JDK 17, API 36 and the checked-in Gradle wrapper. The current Android suite contains **18 unit tests**; the latest local `testDebugUnitTest + lintDebug + assembleRelease` pass succeeded. A first lint pass exposed one release-manifest error caused by `SEND_SMS` implicitly requiring telephony hardware; `35a9250` now declares telephony optional, and also explicitly excludes app data from Android 12+ cloud/device-transfer extraction while retaining `allowBackup=false` / legacy backup disablement. Lint is now error-free; remaining warnings are non-blocking, including the already-known Play policy warning for the battery-optimization exemption and localization/dependency cleanup. Coverage includes outbox persistence/recovery/retention, backend-state preferences, SMS recipient normalization/configuration, fallback eligibility/idempotence/clearing, critical retry policy and critical-first replay ordering. Server suite is **9/9 green**. Production signing remains intentionally unconfigured. `git diff --check` passed before commits.
...
- **Next:** (1) continue remaining Android/OxygenOS reliability checks, especially reboot/recovery, long screen-off runs and network transitions; (2) verify the SMS input/scroll UX once more during ordinary manual use after the fix; (3) release/signing/configuration hardening for restaurant handoff; (4) configure and test the normal outbound alert delivery path, which is still unconfigured on the live backend; (5) optionally build the reusable workstation-side ADB helper described above for future Android projects.
